    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-card text-card-foreground p-4 rounded-xl border border-border shadow-sm">
        <div className="flex items-center gap-3">
          <Link href="/admin/re-exam-portal" className="p-1.5 hover:bg-muted rounded-full transition-colors border border-border">
        ...
        <div className="flex items-center gap-3">
          <span className={cn('px-3 py-1 rounded-full text-xs font-bold uppercase border',
            status === 'Pass' ? 'bg-emerald-100 dark:bg-emerald-950/45 text-emerald-800 dark:text-emerald-300 border-emerald-250 dark:border-emerald-900/40'
      ...
      <div className="bg-card text-card-foreground rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-border bg-gradient-to-r from-destructive/5 to-orange-500/5 dark:from-destructive/10 dark:to-orange-500/10">
          ...
          <p className="text-[10px] text-muted-foreground mt-1">Only showing outcomes where the student failed. Pass marks remain unchanged.</p>
        </div>
        <Table>
          <TableHeader>
            <TableRow className="bg-gradient-to-r from-muted/50 to-muted/80 hover:bg-muted/60">
              ...
              <TableHead className="px-4 py-3 text-[10px] font-bold text-emerald-750 dark:text-emerald-400 uppercase tracking-wider text-center border-r border-border bg-emerald-50/30 dark:bg-emerald-950/10 h-auto" colSpan={2}>Re-Exam Assessment</TableHead>
            ...
            <TableRow className="bg-muted/40 text-[9px] font-semibold text-muted-foreground uppercase hover:bg-muted/50">
              ...
              <TableHead className="px-3 py-2 text-center border-r border-border bg-emerald-50/40 dark:bg-emerald-950/10 h-8"><Calendar className="w-3 h-3 inline mr-1" />Date</TableHead>
                ...
                <TableRow key={lo.name} className="hover:bg-muted/20">
                  ...
                  <TableCell className="px-3 py-4 text-center border-r border-border bg-emerald-50/30 dark:bg-emerald-950/5">
                    <Input type="number" min={0} max={max} step="any" value={m?.reExamMark ?? ''} placeholder="—"
                      ...
                      className={cn('w-16 text-center text-sm font-bold h-9',
      ...
      <div className="bg-card text-card-foreground rounded-xl border border-border p-5 flex flex-wrap items-center gap-8 shadow-sm">
        ...
        <div className="border-l border-border pl-8">
          ...
          <span className={cn('px-3 py-1 rounded-full text-sm font-bold uppercase border',
            status === 'Pass' ? 'bg-emerald-100 dark:bg-emerald-950/45 text-emerald-800 dark:text-emerald-300 border-emerald-250 dark:border-emerald-900/40'
            : status === 'Fail' ? 'bg-destructive/10 text-destructive border-destructive/20'
            : 'bg-muted text-muted-foreground border-border'
          )}>{status}</span>
