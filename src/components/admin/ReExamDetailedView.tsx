...
export default function ReExamDetailedView({ student, evaluation }: Props) {
  ...
  useEffect(() => {
    ...
    resultsData.forEach(r => {
      const name = r.evaluationTemplate?.name ?? evaluation.learningOutcomes[0]?.name ?? 'Mark';
      outcomeMarks[name] = {
  ...
  const updateMark = useCallback((outcomeName: string, patch: Partial<OutcomeMark>) => {
    setLocalMarks(prev => {
      const existing = prev?.outcomeMarks[outcomeName] ?? {
        regularMark: null, regularDate: '', supportMark: null, supportDate: '',
        reExamMark: null, reExamDate: '', remarks: '',
      };
      return {
    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-card p-4 rounded-xl border border-slate-200 dark:border-border shadow-sm">
        <div className="flex items-center gap-3">
          <Link href="/admin/re-exam-portal" className="p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors border border-slate-200 dark:border-border">
        ...
        <div className="flex items-center gap-3">
          ...
          {autoSaveStatus === 'saving' && <span className="text-xs text-slate-500 dark:text-slate-400 animate-pulse">Saving...</span>}
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900">
                ...
                <th className="px-4 py-3 text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider text-center border-r border-slate-200 dark:border-border bg-emerald-50 dark:bg-emerald-950/20" colSpan={2}>Re-Exam Assessment</th>
              ...
              <tr className="bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-border text-[9px] font-semibold text-slate-500 uppercase">
                ...
                <th className="px-3 py-2 text-center border-r border-slate-200 dark:border-border"></th>
                <th className="px-3 py-2 text-center border-r border-slate-200 dark:border-border bg-emerald-50 dark:bg-emerald-950/20"><Calendar className="w-3 h-3 inline mr-1" />Date</th>
                ...
                return (
                  <tr key={lo.name} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="px-3 py-4 text-center border-r border-slate-200 dark:border-border bg-red-50/50 dark:bg-red-950/10">
                      <span className="px-2 py-1 rounded font-bold font-mono text-sm bg-red-100 dark:bg-red-950/40 text-red-800 dark:text-red-300">{originalMark}</span>
                    </td>
                    <td className="px-3 py-4 text-center border-r border-slate-200 dark:border-border bg-emerald-50/50 dark:bg-emerald-950/10">
                      <input type="date" value={m?.reExamDate ?? ''} onChange={e => handleReExamDate(lo.name, e.target.value)}
                    ...
                    <td className="px-3 py-4 text-center border-r border-slate-200 dark:border-border bg-emerald-50/50 dark:bg-emerald-950/10">
                      <input type="number" min={0} max={max} step="any" value={m?.reExamMark ?? ''} placeholder="—"
                        onChange={e => handleReExamMark(lo.name, e.target.value, max)}
                        className={cn('w-16 px-2 py-1.5 text-center text-sm font-bold rounded border-2 focus:outline-none focus:ring-2',
                    ...
                    <td className="px-4 py-4">
                      <input type="text" value={m?.remarks ?? ''} placeholder="Add remarks..." onChange={e => handleRemarks(lo.name, e.target.value)}
      ...
      <div className="bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-border p-5 flex flex-wrap items-center gap-8">
        <div>
          ...
          <p className="text-2xl font-bold text-[#002045] dark:text-blue-300">{obtained} <span className="text-base text-slate-400">/ {fullTotal}</span></p>
        </div>
        <div className="border-l border-slate-200 dark:border-border pl-8">
          ...
          <span className={cn('px-3 py-1 rounded-full text-sm font-bold uppercase',
            status === 'Pass' ? 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300'
            : status === 'Fail' ? 'bg-red-100 dark:bg-red-950/40 text-red-800 dark:text-red-300'
            : 'bg-slate-100 dark:bg-slate-800 text-slate-500'
          )}>{status}</span>
