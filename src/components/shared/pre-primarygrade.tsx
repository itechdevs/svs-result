...
function BackgroundPattern({ schoolName }: { schoolName: string }) {
...
export function buildPrePrimaryData(params: {
    ...
    gradeScales.length > 0
      ? gradeScales
          .map((gs, idx) => ({
            ...
            grade: gs.grade,
            gradePoint:
              gs.gradePoint != null ? formatNum(gs.gradePoint, 2) : "\u2013",
  ...
  if (subjectResults.length > 0) {
    subjects = subjectResults.map((sr) => {
      ...
      return {
        ...
        grade: sr.grade || "\u2013",
        gradePoint:
          sr.gradePoint ?? (gdData ? parseGP(gdData.gradePoint) : null),
    ...
    for (const er of evaluationResults) {
      const tmpl =
        er.evaluationTemplate || templateMap.get(er.evaluationTemplateId);
    ...
    subjects = Object.entries(subjectAgg).map(([, agg]) => {
      ...
      return {
        ...
        grade: gd.grade,
        gradePoint:
          gd.gradePoint ?? (gdData ? parseGP(gdData.gradePoint) : null),
        remarks:
          (gdData ? gdData.description : null) ||
          agg.remarks.join("; ") ||
          null,
      };
    });
  ...
  return {
    ...
    dateOfIssueAD: "",
    rawObservations:
      observationResults.length > 0 ? observationResults : undefined,
    customRemark,
  };
}
